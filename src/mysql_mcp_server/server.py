...
async def read_resource(uri: AnyUrl) -> str:
    ...
    limit = common_config.get("limit", "5")
    uri_str = str(uri)
        ...
        with connect(**config) as conn:
            with conn.cursor() as cursor:
                ...
                if query.strip().upper().startswith("SHOW TABLES"):
                    ...
                    or query.strip().upper().startswith("INSERT")
                    or query.strip().upper().startswith("CREATE")
                    or query.strip().upper().startswith("DROP")
                    or query.strip().upper().startswith("ALTER")
                    or query.strip().upper().startswith("TRUNCATE")
                    or query.strip().upper().startswith("RENAME")
                ):
                    return [TextContent(type="text", text="Unsupport update database")]
